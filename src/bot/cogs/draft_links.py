...
class EndDraftButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if interaction.user.id != self.rooms.author_id and not interaction.user.guild_permissions.manage_guild:
            await first_response_or_followup(interaction, content="Réservé au modérateur/initiatrice.", ephemeral=True)
            return

        # Toujours acquitter avant de supprimer/éditer
        ...
        # Petit accusé de réception (éphémère)
        try:
            await interaction.followup.send("Draft terminée, salons supprimés ✅", ephemeral=True)
        except Exception:
            pass
...
class DraftControlView(discord.ui.View):
    def __init__(self, cog: "DraftLinks", rooms: DraftRooms):
        ...
        self.add_item(EndDraftButton(cog, rooms))
...
class DraftLinks(commands.Cog):
    ...
    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self.active: Dict[int, DraftRooms] = {}

    async def cog_load(self):
        return

    # ---------- utils salons ----------
    async def _create_team_structure(
        self,
        guild: discord.Guild,
        prefix: str,
        ...
        ov = dict(base_ov)
        ov[captain] = discord.PermissionOverwrite(
        ...
        text = await guild.create_text_channel(
            name=f"{prefix}-text", category=category, overwrites=ov, reason="Inhouse draft setup"
        )
        voice = await guild.create_voice_channel(
            name=f"{prefix}-voice", category=category, overwrites=ov, reason="Inhouse draft setup"
        )
        return text, voice
    ...
    async def _grant_user(self, ch: discord.abc.GuildChannel, member: discord.Member):
        try:
            await ch.set_permissions(
                member, view_channel=True, read_message_history=True, send_messages=True, connect=True, speak=True
            )
        except Exception:
            pass
    ...
    async def _delete_rooms(self, guild: discord.Guild, rooms: DraftRooms):
        for cid in [rooms.text_blue_id, rooms.text_red_id, rooms.voice_blue_id, rooms.voice_red_id]:
            ...
            if ch:
                try:
                    await ch.delete(reason="Inhouse draft cleanup")
                except Exception:
                    pass
        cat = guild.get_channel(rooms.category_id)
        if cat:
            try:
                await cat.delete(reason="Inhouse draft cleanup")
            except Exception:
                pass
    ...
    async def _finalize_history(self, guild: discord.Guild, rooms: DraftRooms, manual: bool):
        append_history(
            guild.id,
            {"type": "draft", "title": "inhouse myg", "cap_blue": rooms.captain_blue_id, "cap_red": rooms.captain_red_id, "manual": manual},
        )
    ...
    @app_commands.describe(
        captain_blue="Capitaine Blue",
        captain_red="Capitaine Red",
    ...
    async def draft(
        ...
        lobby_message_id: Optional[str] = None,
    ):
        guild = interaction.guild
        if not guild:
            return await first_response_or_followup(interaction, content="Commande uniquement en serveur.", ephemeral=True)

        # On défère (éphémère) car plusieurs actions réseau vont suivre
        await safe_defer(interaction, ephemeral=True)

        base_ov: Dict[discord.abc.Snowflake, discord.PermissionOverwrite] = {}
        if not everyone_can_view:
            base_ov[guild.default_role] = discord.PermissionOverwrite(view_channel=False)

        try:
            category = await guild.create_category(
                name="inhouse-myg",
                overwrites=base_ov if not everyone_can_view else None,
                reason="Inhouse draft setup",
            )
            ...
            return await first_response_or_followup(interaction, content=f"❌ Impossible de créer les salons/catégorie : {e}", ephemeral=True)
        ...
        # Option: autoriser tous les joueurs du lobby (accès aux deux salons au départ)
        if lobby_message_id:
            try:
                ...
                if lobby:
                    ...
                    for uid in member_ids:
                        ...
                        if m:
                            await self._grant_user(tb, m)
                            await self._grant_user(tr, m)
                            await self._grant_user(vb, m)
                            await self._grant_user(vr, m)
        ...
        try:
            m = await tb.send(embed=ctrl, view=view)
            rooms.message_id = m.id
            await tr.send("Salon **red** prêt. En attente des liens ProDraft…")
        except Exception:
            pass

        await first_response_or_followup(interaction, content=f"✅ Catégorie et salons créés — {category.name}", ephemeral=True)
    ...
    async def draft_set_links(self, interaction: discord.Interaction, blue_url: str, red_url: str, spec_url: str):
        ...
        if not rooms:
            return await first_response_or_followup(interaction, content="Aucune draft active. Lance d'abord `/draft`.", ephemeral=True)

        # Défère éphémère puis DM + post dans salons
        await safe_defer(interaction, ephemeral=True)

        rooms.blue_url = blue_url.strip()
        rooms.red_url = red_url.strip()
        rooms.spec_url = spec_url.strip()
        ...
        try:
            ...
            if b:
                await b.send(f"**Lien Blue ProDraft** : {rooms.blue_url}")
        except Exception:
            pass
        try:
            r = g.get_member(rooms.captain_red_id)
            if r:
                await r.send(f"**Lien Red ProDraft** : {rooms.red_url}")
        except Exception:
            pass

        # Post lien spectator dans les deux salons text
        ...
        await first_response_or_followup(interaction, content="Liens enregistrés et distribués ✅", ephemeral=True)

    @app_commands.command(name="draft_add", description="Donner accès d'équipe à un joueur.")
    @app_commands.describe(user="Membre à ajouter", team="Choisir l'équipe")
    ...
    async def draft_add(self, interaction: discord.Interaction, team: app_commands.Choice[str], user: discord.Member):
        rooms = self.active.get(interaction.guild_id)
        if not rooms:
            return await first_response_or_followup(interaction, content="Pas de draft active.", ephemeral=True)
        ...
        if target:
            await self._grant_user(target, user)
        if voice:
            await self._grant_user(voice, user)

        await first_response_or_followup(interaction, content=f"✅ {user.mention} ajouté côté **{team.value}**", ephemeral=True)

    @app_commands.command(name="draft_remove", description="Retirer l'accès d'équipe à un joueur.")
    @app_commands.describe(user="Membre à retirer", team="Choisir l'équipe")
    @app_commands.choices(
        team=[
            app_commands.Choice(name="blue", value="blue"),
            app_commands.Choice(name="red", value="red"),
    ...
    async def draft_remove(self, interaction: discord.Interaction, team: app_commands.Choice[str], user: discord.Member):
        ...
        try:
            if text:
                await text.set_permissions(user, overwrite=None)
            if voice:
                await voice.set_permissions(user, overwrite=None)
        except Exception:
            pass

        await first_response_or_followup(interaction, content=f"✅ {user.mention} retiré de **{team.value}**", ephemeral=True)
